...
#define error(...) do {ACS_BeginLog(); __nprintf(__VA_ARGS__); ACS_EndLog(); ACS_Exit_Normal(0);} while(0)

static __str InputStr = s"", LastInputStr = s"";
static int InputIdx = -1;

static char internal_getchar()
{
    if(InputIdx >= ACS_StrLen(InputStr))
        return EOF;

    return InputStr[InputIdx++];
}

#define getchar() internal_getchar()
#define ungetc(...) (InputIdx--)
...
static inline Obj *forward(Obj *obj) {
    ...
    ptrdiff_t offset = (unsigned char *)obj - (unsigned char *)from_space;
    ...
    scan2 = (Obj *)((unsigned char *)scan2 + obj->size);
...
static void *alloc_semispace() {
    return calloc(1, MEMORY_SIZE);//mmap(NULL, MEMORY_SIZE, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANON, -1, 0);
}
...
static void gc(void *root) {
    ...
    while (scan1 < scan2) {
        ...
        scan1 = (Obj *)((unsigned char *)scan1 + scan1->size);
    }

    // Finish up GC.
    free(from_space);
    size_t old_nused = mem_nused;
    mem_nused = (size_t)((unsigned char *)scan1 - (unsigned char *)memory);
    if (debug_gc)
...
// (* <integer> ...)
static Obj *prim_star(void *root, Obj **env, Obj **list) {
    int mul = 1;
    for (Obj *args = eval_list(root, env, list); args != Nil; args = args->cdr) {
        if (args->car->type != TINT)
            error("* takes only numbers");
...
// (/ <integer> ...)
static Obj *prim_slash(void *root, Obj **env, Obj **list) {
    Obj *args = eval_list(root, env, list);
    if (length(args) != 2)
        error("malformed /");
    Obj *x = args->car;
...
static void define_primitives(void *root, Obj **env) {
    ...
    add_primitive(root, env, "*", prim_star);
    add_primitive(root, env, "/", prim_slash);
...
ACS_NativeDeclFull(148, 0, __str, ML_GetChatMessage, (int, int));

[[call("ScriptS"), script("event")]]
bool GetMsg (int type, int plr, int isTeam)
{
    if(type != GAMEEVENT_CHAT)
        return true;

    __str s = ACS_ML_GetChatMessage(plr, 0);

    if(s[0] != '$')
        return true;

    InputStr = ACS_StrMid(s, 1, INT_MAX);
    InputIdx = 0;
